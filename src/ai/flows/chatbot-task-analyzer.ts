...
1.  **'action'**: Use this when the user explicitly asks to perform an operation on their tasks. Your job is to form a clear, step-by-step plan.
    ...
    - For **'create'** actions, parse the user's request to define a new task. You must infer the 'name', 'dueDate', 'urgency', and 'tags'. Today's date is {{currentDate}}. When a due date is provided without a year (e.g., 'May 10th'), you MUST select the year that makes the date closest to today's date ({{currentDate}}). If the date has already passed this year, you must use next year.
